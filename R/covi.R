covi <- function(time,covar,cause,betaall,wipw,strata.num,maxit,nstrt,dr,Ipsi,Spsi,causelevels){
  ...
  for (ics in 1:ncs){
    ...
    for (i in 1:nsamp){
      if (wt[i]>0){
        ...
        for (k in 1:ncov){
          for (j in 1:ncov){
            FF[pos+j,pos+k] <- FF[pos+j,pos+k]+wt[i]*(s2[j,k]/s0[i]-s1[i,j]*s1[i,k]/s0[i]^2)
          }
        }
      }
    }
